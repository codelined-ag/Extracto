...
import { downloadGoogleDriveFile } from "@/lib/integrations/google-drive";
...
export const POST = withMutationAuth("ocr:submit", async (request: NextRequest, { auth }) => {
  ...
  const downloaded = await downloadGoogleDriveFile(auth.userId, fileId);
  if (downloaded.bytes.byteLength > MAX_IMPORT_BYTES) {
    throw new ApiRouteError(
      `Google Drive file is too large (${Math.round(downloaded.bytes.byteLength / (1024 * 1024))} MiB > ${MAX_IMPORT_BYTES / (1024 * 1024)} MiB)`,
      413,
    );
  }
  const mime = downloaded.contentType.split(";")[0].trim().toLowerCase();
  if (!ACCEPTED_MIMES.has(mime)) {
    throw new ApiRouteError(`Unsupported file type from Google Drive: ${mime}`, 415);
  }
